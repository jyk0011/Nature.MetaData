...
namespace Nature.MetaData.ControlExtend
{
    ...
    /// 密码框的扩展
    /// </summary>
    public class TextBoxPassExtend : BaseTextBoxExtend
    {
        #region 属性
        ...
        //private readonly int _findWidth  ;
        ///// <summary>
        ...
        public TextBoxPassExtend(Dictionary<string, string> dicControlExt)
            : base(dicControlExt)
        {
            /*
             * string key = "modWidth";
            if (dicControlExt.ContainsKey(key))
                ...
                Functions.MsgBox("没有找到密码框配置信息的" + key + "！<BR>", true);
             ...
             * 多行文本框在查询的时候，调用的是单行文本框，所以下面这段没有
            key = "findWidth";
            if (dicControlExt.ContainsKey(key))
